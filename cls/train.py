...
parser.add_argument('--attn-scale',type=float, default=None, metavar='N',
                    help='spiking self attention scale')
parser.add_argument('--attn-drop',type=float, default=None, metavar='N',
...
parser.add_argument('--tau',type=float, default=2.0, metavar='N',
                    help='time constant tau in neuron')
parser.add_argument('--threshold', type=float, default=1.0, metavar='N',
                    help='neuron threshold')
parser.add_argument('--act-function', type=str, default="SigmoidGrad", metavar='N',
...
parser.add_argument('--dataset', metavar='NAME', default='torch/cifar10',
...
def main():
    ...
    if args.model_ema:
        ...
        model_ema = ModelEmaV2(
            model, decay=args.model_ema_decay, device='cpu' if args.model_ema_force_cpu else device)
        if args.resume:
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if saver is not None:
                ...
                _logger.info(color_text('*** Best metric: {0} (epoch {1})'.format(best_metric, best_epoch),"33"))
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                        log_name, batch_idx, last_idx, batch_time=batch_time_m,
                        loss=losses_m, top1=top1_m, top5=top5_m),"36")
                    )
