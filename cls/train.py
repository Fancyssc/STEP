...
parser.add_argument(
    "--config",
    default="configs/spikformer/cifar10.yml",
    type=str,
    metavar="FILE",
    help="YAML config file specifying model arguments",
)
...
parser.add_argument(
    "--depths", type=int, default=4, metavar="layer", help="model depths (default: 4)"
)
parser.add_argument(
    "--num-classes",
    type=int,
...
parser.add_argument(
    "--num_heads", type=int, default=None, metavar="N", help="attention head number"
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=None,
    metavar="N",
    help="spiking self attention scale",
)
parser.add_argument(
    "--attn-drop", type=float, default=None, metavar="N", help="attention drop rate"
)
parser.add_argument(
    "--mlp-drop", type=float, default=0.0, metavar="N", help="mlp drop rate"
)
...
parser.add_argument(
    "--embed-layer", type=str, default="SPS", metavar="N", help="embedding method"
)
parser.add_argument(
    "--attn-layer", type=str, default="SSA", metavar="N", help="attention method"
)
...
parser.add_argument(
    ...
    default="LIFNode",
    metavar="N",
    help="neuron node type in the model",
)
parser.add_argument(
    "--tau", type=float, default=2.0, metavar="N", help="time constant tau in neuron"
)
parser.add_argument(
    "--threshold", type=float, default=1.0, metavar="N", help="neuron threshold"
)
parser.add_argument(
    "--act-function",
    type=str,
...
parser.add_argument(
    "--alpha", type=float, default=4.0, metavar="N", help="surrogate gradient alpha"
)
...
parser.add_argument(
    "--data-dir",
    metavar="DIR",
    default="/data0/datasets/CIFAR10",
...
parser.add_argument(
    "--event-mix", action="store_true", help="EventMix for event data (default: False)"
)
parser.add_argument(
    "--mixup",
    type=float,
...
parser.add_argument(
    "--output",
    default="/home/shensicheng/log/spikformer_briancogModel_spikingjellyScript",
    type=str,
    metavar="PATH",
...
def main():
    ...
    #
    # #change PyTorch default cuda config
    ...
    create_model_args = {
        key: all_args[key] for key in create_model_keys if key in all_args
    }
    create_model_args["node"] = node_type  # node type
    create_model_args["act_func"] = act_function  # surrogate gradient

    # create model
    model = create_model(model_name=create_model_args["model"], **create_model_args).to(
        args.device
    )
        ...
        if args.experiment:
            ...
            exp_name = "--".join(
                [
                    ...
                    str(args.dataset).split("/")[-1],  # 防止创建奇怪的路径
                    args.node_type,
                    "thres_" + str(args.threshold),
                    "tau_" + str(args.tau),
                    "seed_" + str(args.seed),
                    "epoch_" + str(args.epochs),
                    datetime.now().strftime("%Y%m%d-%H%M%S"),  # 防止覆盖
                    # str(data_config['input_size'][-1])
                ]
            )
    ...
    if best_metric is not None:
        _logger.info(
            color_text(
                "*** Best metric: {0} (epoch {1})".format(best_metric, best_epoch), "33"
            )
        )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if "mnist" in args.dataset.lower() or "smnist" in args.dataset.lower():
            weights = torch.tensor([0.299, 0.587, 0.114], device=input.device).view(
                1, 3, 1, 1
            )
        ...
        if "smnist" == args.dataset.lower():
            ...
            if not hasattr(args, "permutation_loaded"):
                ...
                if os.path.exists(perm_path):
                    ...
                    if batch_idx == 0:
                        _logger.info(
                            f"Created and saved new permutation to {perm_path}"
                        )
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if "scifar" in args.dataset.lower():
                ...
                if args.in_channels == 1:
                    ...
                    # concat R G B channel
                    input = torch.cat([r, g, b], dim=1).permute(
                        0, 2, 1
                    )  # [B, 1, 3072 ]
