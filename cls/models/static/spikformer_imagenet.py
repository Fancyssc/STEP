...
class SSA(BaseModule):
    ...
    def forward(self, x, res_attn):
        ...
        q = q_conv_out.transpose(-1,-2).reshape(-1, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()  # (TB,num_heads,N,C//num_heads)

        k_conv_out = self.k_conv(x_for_qkv)
        k_conv_out = self.k_bn(k_conv_out)
        k_conv_out = self.k_lif(k_conv_out)
        k = k_conv_out.transpose(-1,-2).reshape(-1, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()

        v_conv_out = self.v_conv(x_for_qkv)
        v_conv_out = self.v_bn(v_conv_out)
        v_conv_out = self.v_lif(v_conv_out)
        v = v_conv_out.transpose(-1,-2).reshape(-1, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()

        attn = (q @ k.transpose(-2, -1)) * self.scale
        x = attn @ v
        x = x.transpose(2, 3).reshape(TB, C, N).contiguous()
        x = self.attn_lif(x) # TB C N
        x = self.proj_lif(self.proj_bn(self.proj_conv(x))).reshape(TB, C, H, W).contiguous()
...
class Block(nn.Module):
    def __init__(self, embed_dim=384, num_heads=12, step=4,mlp_ratio=4. ,attn_scale=0.125, attn_drop=0.,mlp_drop=0.,node=LIFNode,tau=2.0,threshold=1.0,act_func=SigmoidGrad, alpha=4.,layer_by_layer=True,norm_layer=nn.LayerNorm,drop_path=0.):
        super().__init__()
...
class Spikformer(BaseModule):
    def __init__(self,
                 ...
                 depths=4, node=LIFNode,tau=2.0,threshold=1.0,act_func=SigmoidGrad, alpha=4.,layer_by_layer=True,drop_path_rate=0.
                 ):
        ...
        block = nn.ModuleList([Block(
            ...
            node=node,act_func=act_func,tau=tau,threshold=threshold,alpha=alpha,drop_path=dpr[j])
