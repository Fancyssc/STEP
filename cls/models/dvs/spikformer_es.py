...
class MLP(BaseModule):
    ...
    def forward(self, x):
        ...
        x = self.fc1_bn(x)
        tb,c,n = x.shape
        x = self.fc1_lif(x.reshape(tb,c*n)).reshape(tb,c,n)

        x = self.fc2_conv(x)
        x = self.fc2_bn(x)
        tb,c,n = x.shape
        x = self.fc2_lif(x.reshape(tb,c*n)).reshape(tb,c,n)
...
class SSA(BaseModule):
    ...
    def forward(self, x):
        ...
        tb,c,n = q_conv_out.shape
        q_conv_out = q_conv_out.reshape(tb,c*n)
        q_conv_out = self.q_lif(q_conv_out).reshape(tb,c,n).transpose(-2, -1) #TB N C
        ...
        tb,c,n = k_conv_out.shape
        k_conv_out = k_conv_out.reshape(tb,c*n)
        k_conv_out = self.k_lif(k_conv_out).reshape(tb,c,n).transpose(-2, -1)
        ...
        tb,c,n = v_conv_out.shape
        v_conv_out = v_conv_out.reshape(tb,c*n)
        v_conv_out = self.v_lif(v_conv_out).reshape(tb,c,n).transpose(-2, -1)
        ...
        x = x.flatten(0, 1)
        tb,c,n = x.shape
        x = self.attn_lif(x.reshape(tb,c*n)).reshape(tb,c,n)
        x = self.proj_bn(self.proj_conv(x))
        tb,c,n = x.shape
        x = self.proj_lif(x.reshape(tb,c*n)).reshape(tb,c,n) # TB C N
...
@register_model
def spikformer_es(pretrained=False, **kwargs):
    model = Spikformer(
