...
class SPS(BaseModule):
    ...
    def __init__(self, step=4, encode_type='direct', img_h=32, img_w=32, patch_size=4, in_channels=3,
        ...
        # 定义一个 learnable 的时间步权重向量，
        # 它的 shape 为 (step, )，用于对每个时间步的输出特征进行加权
        self.time_weights = Parameter(torch.ones(step))


        self.proj_conv = DepthwiseSeparableConv(in_channels, embed_dims // 8, kernel_size=3, stride=1, padding=1,
                                                bias=False)
        self.proj_bn = nn.BatchNorm2d(embed_dims // 8)
        self.proj_lif = node(step=step, tau=tau, act_func=act_func(alpha=alpha),
                             threshold=threshold, layer_by_layer=layer_by_layer, mem_detach=False)

        self.proj_conv1 = DepthwiseSeparableConv(embed_dims // 8, embed_dims // 4, kernel_size=3, stride=1, padding=1,
                                                 bias=False)
        self.proj_bn1 = nn.BatchNorm2d(embed_dims // 4)
        self.proj_lif1 = node(step=step, tau=tau, act_func=act_func(alpha=alpha),
                              threshold=threshold, layer_by_layer=layer_by_layer, mem_detach=False)

        self.proj_conv2 = DepthwiseSeparableConv(embed_dims // 4, embed_dims // 2, kernel_size=3, stride=1, padding=1,
                                                 bias=False)
        self.proj_bn2 = nn.BatchNorm2d(embed_dims // 2)
        self.proj_lif2 = node(step=step, tau=tau, act_func=act_func(alpha=alpha),
                              threshold=threshold, layer_by_layer=layer_by_layer, mem_detach=False)
        self.maxpool2 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.proj_conv3 = DepthwiseSeparableConv(embed_dims // 2, embed_dims, kernel_size=3, stride=1, padding=1,
                                                 bias=False)
        self.proj_bn3 = nn.BatchNorm2d(embed_dims)
        self.proj_lif3 = node(step=step, tau=tau, act_func=act_func(alpha=alpha),
                              threshold=threshold, layer_by_layer=layer_by_layer, mem_detach=False)
        self.maxpool3 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.rpe_conv = DepthwiseSeparableConv(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.rpe_bn = nn.BatchNorm2d(embed_dims)
        self.rpe_lif = node(step=step, tau=tau, act_func=act_func(alpha=alpha),
                            threshold=threshold, layer_by_layer=layer_by_layer, mem_detach=False)

    def forward(self, x):

        self.reset()
        TB, C, H, W = x.shape  # 注意：输入的第一维 TB = T * B
        ...
        x_feat = x  # TB, embed_dims, H', W'
        x = self.rpe_conv(x)
        x = self.rpe_bn(x)
        x = self.rpe_lif(x)
        x = x + x_feat

        # 将 TB 维度分解为 (step, B)，其中 B = TB // step
        B = TB // self.step
        # 先记录 x 原有形状 (TB, C, H', W')
        C_new, H_new, W_new = x.shape[1], x.shape[2], x.shape[3]
        # reshape 成 (step, B, C, H', W')
        x = x.view(self.step, B, C_new, H_new, W_new)
        # 对每个 time step 应用对应的 learnable 权重：
        # 首先将 self.time_weights 调整 shape 为 (step, 1, 1, 1, 1)
        time_weight = self.time_weights.view(self.step, 1, 1, 1, 1)
        x = x * time_weight
        # 再恢复 TB 维度，即 reshape 成 (TB, C, H', W')
        x = x.view(TB, C_new, H_new, W_new)

        # flatten spatial dimensions为序列并转置，输出 (TB, N, embed_dims)
        x = x.flatten(-2).transpose(-1, -2)  # TB, N, embed_dims

        return x  # TB, N, embed_dims
